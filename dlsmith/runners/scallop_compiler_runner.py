...
class ScallopCompilerRunner(BaseRunner):


    def run_executable(self, orig):
        """
            The compiler will produce an executable which needs to be executed and the result processed. 
            We will use the processor from the original Scallop compiler. 
        """
        if orig:
            command = self.program.get_program_file_path()[:-4]
        ...
        return standard_error, standard_output
    ...
    def run_original(self, program, engine_options, logger):
        ...
        command = "timeout -s SIGKILL " + str(self.params["timeout"]) + "s " + self.params["path_to_scallop_compiler"] + " " + self.program.get_program_file_path()
        logger.add_log_text("[Scallop runner] Running the command: " + command)
        p = subprocess.Popen(command, shell=True, stderr=subprocess.PIPE, stdout=subprocess.PIPE)
        standard_error = p.stderr.read().decode()
        standard_output = p.stdout.read().decode()
        
        # Delete all the compiler related trash
        compiler_trash = os.path.join(self.program.get_program_path(), "." + self.program.get_program_file_name() + ".sclcmpl")
        os.system("rm -rf " + compiler_trash)
        os.system("rm -rf " + compiler_trash)
        os.system("rm -rf " + compiler_trash)
        ...
        # Check if the executable file has been successfully generated
        if os.path.exists(self.program.get_program_file_path()[:-4]): logger.add_log_text("[Scallop runner] Executable " + self.program.get_program_file_name() + " successfully generated!")
        else: return 2

        # Extra step for Scallop compiler
        standard_error, standard_output = self.run_executable(True)
    ...
    def run_transformed(self, program, engine_options, logger):
        ...
        # Extra step for Scallop compiler
        standard_error, standard_output = self.run_executable(False)
    ...
    def process_standard_error(self, std_err, logger):
        ...
        if std_err.find("thread 'main' panicked at") != -1:
            print("MAIN THREAD PANICKED")
            return 2
        
        if std_err.find("Cannot stratify program:") != -1:
            return 2

        if std_err.find("error[E0382]") != -1:
            return 2
        
        if std_err.find("failed to get `proc-macro2`") != -1:
            return 2
        
        if std_err.find("spurious network error") != -1:
            return 2
        
        if std_err.find("failed to select a version for the requirement") != -1:
            return 2
    ...
    def process_standard_output(self, std_output, logger): 
        ...
        if std_output.find("error[E0382]") != -1:
            return 2
        
        if std_output.find("failed to get `proc-macro2`") != -1:
            return 2

        if std_output.find("spurious network error") != -1:
            return 2
        
        if std_output.find("failed to select a version for the requirement") != -1:
            return 2
